...
def generate_triangle_template_time(times, period, lag, asymmetry=0.5):
    ...
    """
    peak_time = (lag + asymmetry * period) % period
...
def run_Cosine_Kendall(series, period_range=range(20, 28), interval=1):
    y = series.rank().values
    n = len(y)
    best_p = 1.0
    best_tau = 0.0
...
def fit_group_cosinor(df, period_list=[24]):
...
class CircadianApp(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        about_menu.addAction("Developer", self.show_Acknowlegements)
    ...
    def show_Notes(self):
        QMessageBox.about(
            ...
            "- - - - Feedback - - - -\n"
            "Eamil me < binbinwu.phd@gmail.com >")
    ...
    def run_analysis(self):
        try:
            ...
            for group, series in df.items():
                ...
                cos_res_df = fit_group_cosinor(cos_df,period_list=[20, 21, 22, 23, 24, 25, 26, 27, 28])
    ...
    def run_pythonJTK_analysis(self):
        try:
            dtype, ok = QInputDialog.getItem(
                self, "Select Dataset", "Which behavior?",
                ["Behavior_1", "Behavior_2", "Behavior_3"], 0, False)
            if not ok:
                return
            ...
            self.status.setText(df_out.to_string(index=False))
