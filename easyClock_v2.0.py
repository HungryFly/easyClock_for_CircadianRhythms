 ...
 # plot_cosinor_fitting_model under visualization functionality.
    def plot_cosinor_fitting_model(self, dtype=None, groups_to_plot=None, time_range=None):
        ...
        for group in groups_to_plot:
            ...
            ax.plot(time_eval, fit_curve, label=f"{group} Cosinor Fit", color='black')
            ax.scatter(times, values, label=f"{group} Mean", color='gray', marker='o', alpha=0.6)

        ax.set_xlabel("Time")
        ax.set_ylabel("Value")
        ax.set_title(f"Cosinor Fitting Model: {dtype}")
        ax.legend()
        ax.grid(False)
        plt.tight_layout()
        plt.show()
 ...
 # plot_cosinor_kendall_fitting_model within visualization functionality.
    def plot_cosinor_kendall_fitting_model(self, dtype=None, groups_to_plot=None, time_range=None):
        ...
        for group in groups_to_plot:
            ...
            ax.plot(time_eval, fit_curve, label=f"{group} Kendall Fit", color='black')
            ax.scatter(times, values, label=f"{group} Mean", color='gray', marker='o', alpha=0.6)

        ax.set_xlabel("Time (hr)")
        ax.set_ylabel("Value")
        ax.set_title(f"Cosine-Kendall Fitting Model: {dtype}")
        ax.legend()
        ax.grid(False)
        plt.tight_layout()
        plt.show()
 ...
 # plot_python_jtk_fitting_model within visualization functionality.
    def plot_python_jtk_fitting_model(self, dtype=None, groups_to_plot=None, time_range=None):
        ...
        for i, group in enumerate(groups_to_plot):
            ...
            plt.plot(time, raw_data, 'o', label="Raw", alpha=0.5, color='gray')
            plt.plot(time, fit_curve, '-', label="JTK Fit", color="black")
            plt.title(f"{group} ({dtype})")
            plt.xlabel("Time")
            plt.ylabel("Value")
            plt.legend()
            plt.grid(True)
...
# plot_harmonic_cosinor_fitting_model within visualization functionality.
    def plot_harmonic_cosinor_fitting_model(self, dtype=None, groups_to_plot=None, time_range=None):
        ...
        for group in groups_to_plot:
            ...
            ax.plot(t_grid_full, model_wave_full, label=f"{group} Harmonic Fit", color='black', lw=2)
            ax.scatter(times, values, label=f"{group} Mean", color='gray', alpha=0.5)

        ax.set_xlabel("Time")
        ax.set_ylabel("Value")
        ax.set_title(f"Harmonic-Cosinor Fit: {dtype}")
        ax.legend()
        ax.grid(False)
        plt.tight_layout()
        plt.show()
